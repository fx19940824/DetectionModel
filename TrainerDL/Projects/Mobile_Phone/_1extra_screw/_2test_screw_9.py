...
with torch.no_grad():
    ...
    net = torch.jit.load("./screw_model/screw_model6.pt")
    device = torch
    # net.cuda().half()
    net.cuda()
    net.eval()
    ...
    bias = cv2.imread(os.path.join(os.getcwd() + "/screw_model/screw_bias.bmp"), 0)
    bias = 0.0 * (bias.astype(np.float32) + 1) * 20.0 / 255.0
    ...
    label_dir = ""
    #
    # left_image_dir = "/home/cobot/mobile_data/screw_train/left4"
    # right_image_dir = "/home/cobot/mobile_data/screw_train/right4"
    # label_dir = "/home/cobot/mobile_data/screw_train/label4"
    ...
    for name in names:
        ...
        # left_imagex = np.clip(left_imagex, 0, 255).astype(np.uint8)
        ...
        out = net(x1=left_image, temp=temp)
        out[:, 0, :, :] = out[:, 0, :, :] + bias
