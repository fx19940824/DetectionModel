...
import shutil
import sys
from data_set_maker_screw2 import make_lmdb
from ctools.basic_func import image_correct
...
def solver_maker(solver_file, net_file, maxT, prefix):
    ...
    sovler_string['weight_decay'] = str(0.000000)  # 权重衰减 0.000002
    sovler_string['lr_policy'] = '"step"'  # 学习策略
    sovler_string['stepsize'] = str(2000)  # 2000
    sovler_string['gamma'] = str(0.75)  # 的
...
end_iter_K = 2
# ---------------------------网络1-----------------------------------------------------------------------
for iter in range(start_iter_K, end_iter_K):
    # # 生成数据
    if iter < 1:
        temp = cv2.imread(root + "/data/3templates/2.png")
        make_lmdb(temp, case=1)
...
for iter in range(start_iter_K, end_iter_K):
    # 生成数据
    if iter < 1:
        temp0 = cv2.imread(root + "/data/3templates/2.png")
        temp = cv2.imread(root + "/data/3templates/3.png")
        temp, M = image_correct(temp0, temp)
        make_lmdb(temp, case=2)
