...
def make_lmdb(template, case=1, types="all_image"):
    # ---指定各种文件夹-------------------------------------------------------------------------------------------------------
    # 输入图像文件位置
    ...
    if case == 1:
        image_lmdb = root + "/data/4screw/train1_lmdb/image_lmdb"
        label_lmdb = root + "/data/4screw/train1_lmdb/label_lmdb"
    else:
        image_lmdb = root + "/data/4screw/train2_lmdb/image_lmdb"
        label_lmdb = root + "/data/4screw/train2_lmdb/label_lmdb"
    ...
    env_label1 = lmdb.open(label_lmdb, map_size=1e12)
    ...
    for short_file_name, k in zip(files, range(len(files))):
        ...
        # 分区内截取ROI,主要是图太大了显存搞不定,训练效果也不好
        rois = [[0, 512 + 64, 0, 1280],  # seg_ROIs[0] = Rect(0, 0, 512 + 24, 1280); // 分成4块
                [512, 512 + 768 + 64, 0, 512 + 64],  # seg_ROIs[1] = Rect(512, 0, 768 + 24, 512 + 24);
                [512, 512 + 1948, 512, 512 + 768],  # seg_ROIs[2] = Rect(512, 512, 1948, 768);
                [1280, 1280 + 1180, 0, 512 + 64]]  # seg_ROIs[3] = Rect(1280, 0, 1180, 512 + 24);

        # types = "area1"  # "all_image","area0","area1""area2""area3"
        if types is "all_image":
            steps = np.ceil((np.array(image.shape[:2]) + 128.0) / 512)  # 别问我为啥+128，我也想不起，但是可工作Yong@2018年12月12日
        ...
        if types is "area0":
            steps = np.ceil((np.array([rois[0][3] - rois[0][2], rois[0][1] - rois[0][0]]) + 128.0) / 512)
            ind_x = np.round(np.linspace(rois[0][2], rois[0][3] - 512 - 4, steps[0]))
            ind_y = np.round(np.linspace(rois[0][0], rois[0][1] - 512 - 4, steps[1]))
            ind_x = np.int(np.random.choice(ind_x) + np.random.randint(0, 4))
            ind_y = np.int(np.random.choice(ind_y) + np.random.randint(0, 4))
        if types is "area1":
            steps = np.ceil((np.array([rois[1][3] - rois[1][2], rois[1][1] - rois[1][0]]) + 128.0) / 512)
            ind_x = np.round(np.linspace(rois[1][2], rois[1][3] - 512 - 4, steps[0]))
            ind_y = np.round(np.linspace(rois[1][0], rois[1][1] - 512 - 4, steps[1]))
            ind_x = np.int(np.random.choice(ind_x) + np.random.randint(0, 4))
            ind_y = np.int(np.random.choice(ind_y) + np.random.randint(0, 4))
        if types is "area2":
            steps = np.ceil((np.array([rois[2][3] - rois[2][2], rois[2][1] - rois[2][0]]) + 128.0) / 512)
            ind_x = np.round(np.linspace(rois[2][2], rois[2][3] - 512 - 4, steps[0]))
            ind_y = np.round(np.linspace(rois[2][0], rois[2][1] - 512 - 4, steps[1]))
            ind_x = np.int(np.random.choice(ind_x) + np.random.randint(0, 4))
            ind_y = np.int(np.random.choice(ind_y) + np.random.randint(0, 4))
        if types is "area3":
            steps = np.ceil((np.array([rois[3][3] - rois[3][2], rois[3][1] - rois[3][0]]) + 128.0) / 512)
            ind_x = np.round(np.linspace(rois[3][2], rois[3][3] - 512 - 4, steps[0]))
            ind_y = np.round(np.linspace(rois[3][0], rois[3][1] - 512 - 4, steps[1]))
            ind_x = np.int(np.random.choice(ind_x) + np.random.randint(0, 4))
            ind_y = np.int(np.random.choice(ind_y) + np.random.randint(0, 4))
        # print steps
