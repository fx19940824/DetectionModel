...
# 图像数据路径设置
left_image_dir = "/home/cobot/mobile_data/screw_train/left"
label_dir = "/home/cobot/mobile_data/screw_train/label"
...
# -Step.1 (c) 网络主体结构
class ScrewNet9(torch.jit.Module):
    def __init__(self, in_channel, num_classes, verbose=False):
        super(ScrewNet9, self).__init__()
    ...
    def forward(self, x1, temp):
        x = torch.cat((x1, temp), 1)  # 构成需要的通道数量
...
class ScrewData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img1 = torch.from_numpy(image_loader(l_image_dir).astype(np.float32) + np.random.randint(-2, 2)).cuda()
...
def train(net, train_data, valid_data, num_epochs, optimizer, criterion, mode='original'):
    ...
    for epoch in range(num_epochs):
        ...
        # net = net.eval()
        for im1, label, temp in train_data:
            # if mode == 'augment':
            ...
            if np.random.random() < 1.0:
                output = net(im1, temp)  # 0.12s
            else:
                output = net(im2, temp)  # 0.12s
        ...
        if (epoch + 1) % 1 == 0:
            ...
            trace_script_module = torch.jit.trace(net, (example1, temp))
            trace_script_module.save("./screw_model/screw_model" + str(epoch) + ".pt")
            # trace_script_module.module.save("./screw_model/screw_model" + str(epoch) + ".pt")  # 并行GPU的保存方法
...
def main():
    ...
    # -加载数据loader
    train_data = ScrewData(left_image_dir=left_image_dir, right_image_dir="", label_dir=label_dir,
                           temp_path=temp_path,
    ...
    if os.path.exists("temp6.pth"):
        net = torch.load("temp6.pth")
    else:
        net = ScrewNet9(6, 3, verbose=True)
    # net = ScrewNet9(9, 3, verbose=True)
    ...
    with torch.no_grad():
        ...
        temp = torch.rand(1, 3, 2400, 1200).cuda()
        trace_script_module = torch.jit.trace(net, (left_image, temp))

        trace_script_module.save("./screw_model/ScrewNet.pt")
