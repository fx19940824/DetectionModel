...
right_image_dir = "/media/cobot/94584AF0584AD0A2/data/_0normal_images/right"
patch_dir = "/media/cobot/94584AF0584AD0A2/data/_2screw_patches/Results"

if AREA < 0:
    left_out_dir = "/home/cobot/mobile_data/screw_train/left"
    right_out_dir = "/home/cobot/mobile_data/screw_train/right"
    mask_out_dir = "/home/cobot/mobile_data/screw_train/label"
else:
    left_out_dir = "/home/cobot/mobile_data/screw_train/left" + str(AREA)
    right_out_dir = "/home/cobot/mobile_data/screw_train/right" + str(AREA)
...
def asemble(Max_Num=6000, types="screw"):
    ...
    while True:  # 不停的读取新图像
        ...
        right_image = cv2.imread(os.path.join(right_image_dir, image_list[image_ind]), -1)
        if left_image is None or right_image is None:  # 正常图像读取有问题
            print("Please check the path", os.path.join(left_image_dir, image_list[image_ind]), "and ",
                  os.path.join(right_image, image_list[image_ind]))
            image_ind = image_ind + 1
            continue
        if AREA > -1:
            i = 4 - AREA
            left_image = left_image[:, i * 1550:(i * 1550 + 1520), :]
            right_image = right_image[:, i * 1550:(i * 1550 + 1520), :]
        left_image = left_image + 0.0 + np.random.randint(-2, 2)  # 颜色扰动
        right_image = right_image + 0.0 + np.random.randint(-2, 2)
        np.clip(left_image, 0, 255, left_image)  # 数据裁剪
        np.clip(right_image, 0, 255, right_image)
        ...
        right_output = copy.deepcopy(right_image)  # 用于确定后输出
        ...
        while True:  # 不停的读取新的patch
            ...
            patch2 = cv2.imread(os.path.join(patch_dir, patch_list[patch_ind + 1]), -1)

            if patch1 is None or patch2 is None:  # 螺钉文件不存在,或者有问题
            ...
            if random_flag:
                ...
                patch2 = np.rot90(patch2, k)
                mask2 = np.rot90(mask2, k)
            ...
            patch2 = patch2[:, :, 0:3] + 0.0 + np.random.randint(-10, 10)
            np.clip(patch2, 0, 255, patch2)
            ...
            if assemble_flag:
                ...
                # if random.random() < 0.99:
                right_output = merge(right_output, patch2, mask2, assemble_x, assemble_y)
                mask_output = merge(mask_output, mask2, mask2, assemble_x, assemble_y)
                if batch_flag:
            ...
            if batch_counter == Max_batch:
                ...
                cv2.imwrite(os.path.join(right_out_dir, new_name), right_output)
