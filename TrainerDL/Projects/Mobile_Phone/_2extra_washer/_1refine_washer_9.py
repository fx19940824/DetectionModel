...
# step.1 基础设置
IMG_NUM = 200  # 更新bias用的图像数量，-1表示全部
...
net_path = "./washer_model/washer_model0.pt"
...
for area in [-1, 0, 1, 2, 3, 4]:  # 分区域构建bias
    batch_size = 1
    ...
    with torch.no_grad():
        ...
        for k, name in enumerate(names[0:IMG_NUM]):
            ...
            if area > -1:
                ...
                right_image = right_image[:, (4 - area) * 1550: ((4 - area) * 1550 + 1520), :]
            ...
            right_image = right_image.cuda().half()
            ...
            if k % 1 == 0:
                ...
                xxx[:, :, 0] = data
                if area > -1:
                    cv2.imwrite("./washer_model/washer_bias_" + str(area) + ".bmp", xxx)
                else:
                    cv2.imwrite("./washer_model/washer_bias.bmp", xxx)
