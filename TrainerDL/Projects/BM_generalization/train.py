...
class Generalization_CLS:
    ...
    def train_epoch(self):
        ...
        train_acc = train_acc.float() / train_count
        train_loss /= len(self.dataloader['train'])
        print('Train  Avg Loss: {:.6f}, Acc: {:.6f}'.format(train_loss, train_acc))
    ...
    def eval(self, stage):
        ...
        eval_acc = eval_acc.float() / eval_count
        eval_loss /= len(self.dataloader[stage])
        print('{}  Avg Loss: {:.6f}, Acc: {:.6f}'.format(stage, eval_loss, eval_acc))
        time.sleep(1)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch, self.args.epochs):
            ...
            self.results[epoch]['train'] = self.train_epoch()

            with torch.no_grad():

                print('epoch {} :{}'.format(epoch + 1, "test"))
                self.results[epoch]['test'] = self.eval("test")

                print('epoch {} :{}'.format(epoch + 1, "valid"))
                self.results[epoch]['valid'] = self.eval("valid")
    ...
    def print_option(self):
        ...
        for hyperparam in self.args.__dict__.items():
            print("{0} : {1}".format(hyperparam[0], hyperparam[1]))
...
if __name__ == '__main__':
    gac = Generalization_CLS("/media/cobot/418e5be5-dc8e-4eb7-b81d-a09c5fb6c3ab/Projects/bomo_classify/cfg_chen/bomo-back_xception_train.cfg")
    gac.train()
